...
(function (root, factory) {
  ...
  function parseMasterMpd(text, baseUrl) {
    ...
    while ((m = repRe.exec(text)) !== null) {
      ...
      // Find the parent AdaptationSet's mimeType
      const beforeMpd = text.substring(0, m.index);
      const asMatch = beforeMpd.match(/<AdaptationSet[^>]*mimeType="([^"]+)"/i);
      const mimeType = asMatch ? asMatch[1] : "";
      ...
      // DASH Representations don't have direct URLs — use the base MPD URL
      v.url = baseUrl;
      variants.push(v);
    }
    return variants;
