...
(function (root, factory) {
  ...
  function buildCommand(entry, tool) {
    ...
    if (tool === "ffmpeg") {
      ...
      const hdrs = [];
      if (ref) hdrs.push("Referer: " + ref);
      if (cookie) hdrs.push("Cookie: " + cookie);
      if (hdrs.length) parts.push("-headers " + shq(hdrs.join("\r\n")));
    ...
    if (cookie) parts.push("--add-header " + shq("Cookie: " + cookie));
  ...
  // Parse #EXT-X-MEDIA:TYPE=SUBTITLES entries from an HLS master playlist.
  // Returns array of { url, language, name, isDefault, isAutoselect }.
  function parseMasterSubtitles(text, baseUrl) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      if (!uri) continue;
      let url = uri;
      try { url = new URL(uri, baseUrl).href; } catch (e) { /* keep raw */ }
      const lang = (attrs.match(/LANGUAGE="([^"]+)"/i) || [])[1] || "";
      const name = (attrs.match(/NAME="([^"]+)"/i) || [])[1] || lang || "subtitle";
      const isDefault = /DEFAULT=YES/i.test(attrs);
      const isAutoselect = /AUTOSELECT=YES/i.test(attrs);
      subs.push({ url, language: lang, name, isDefault, isAutoselect, label: "VTT" });
    }
    return subs;
  }

  // Detect language from a VTT text sample using common-word heuristics.
  // Returns an ISO 639-1 code or "" if unclear.
  function detectVttLanguage(text) {
    if (!text) return "";
    const sample = text.slice(0, 2000).toLowerCase();
    // High-confidence character-set tests first.
    ...
    if (/[ğşçöüİ]/.test(sample) && /\b(bir|ve|bu|için|olarak|ama|çok|daha)\b/.test(sample)) return "tr";
    // Check for German before scoring (umlauts common to both tr/de).
    if (/[äöüß]/.test(sample) && /\b(der|die|das|und|ist|nicht|sie|mit|ein|auf|für)\b/i.test(sample)) return "de";
    // Latin-script languages: common-word scoring.
    const words = {
      en: ["the", "and", "you", "that", "this", "with", "have", "from", "not", "but", "was", "are", "can", "will", "been", "they", "for", "all", "were"],
      de: ["der", "die", "und", "ist", "nicht", "das", "sie", "mit", "sich", "ein"],
      es: ["que", "los", "las", "una", "con", "por", "del", "para", "como", "más", "no", "este", "pero", "entre"],
      fr: ["les", "des", "une", "est", "pas", "dans", "sur", "que", "pour", "avec"],
      it: ["che", "non", "una", "sono", "con", "del", "gli", "della", "nella", "come"],
      pt: ["que", "não", "com", "uma", "para", "dos", "como", "mas", "bem", "muito"],
      id: ["yang", "dan", "ini", "itu", "dengan", "akan", "pada", "dari", "tidak", "untuk"],
      ms: ["yang", "dan", "ini", "itu", "dengan", "akan", "pada", "dari", "tidak", "untuk"],
    };
    let best = "", bestScore = 0;
    for (const [lang, ws] of Object.entries(words)) {
      let score = 0;
      for (const w of ws) {
    ...
    // Indonesian/Malay disambiguation
    if (best === "id" || best === "ms") {
      if (/\bbisa\b/.test(sample) && !/\bboleh\b/.test(sample)) return "id";
      if (/\bboleh\b/.test(sample) && !/\bbisa\b/.test(sample)) return "ms";
      return "id";
    }
    return "";
  }
  ...
  // Parse a DASH MPD manifest to extract Representation streams with quality info.
  // Returns array of { url, bandwidth, width, height, codecs, mimeType, label, isAudio }
  function parseMasterMpd(text, baseUrl) {
    const variants = [];
    // Match <Representation .../> or <Representation ...>...</Representation>
    const repRe = /<Representation\s+([^>]*?)(?:\/>|>)/gi;
    let m;
    while ((m = repRe.exec(text)) !== null) {
      ...
      const cs = (attrs.match(/codecs="([^"]+)"/i) || [])[1];
      if (!bw) continue;

      // Find the nearest parent AdaptationSet's mimeType (last one before this Representation)
      const before = text.substring(0, m.index);
      const asIdx = before.lastIndexOf("<AdaptationSet");
      let mimeType = "";
      if (asIdx !== -1) {
        const asMatch = before.substring(asIdx).match(/mimeType="([^"]+)"/i);
        if (asMatch) mimeType = asMatch[1];
      }
      ...
      const v = {
        ...
        codecs: cs || "",
        mimeType: mimeType,
        isAudio: !!(mimeType && mimeType.startsWith("audio/")),
      };
      v.resolution = (v.width && v.height) ? v.width + "x" + v.height : "";
  ...
  // Detect if text is a DASH MPD manifest.
  function isMasterMpd(text) {
    return /<MPD\b/i.test(text) && /<Representation\b/i.test(text);
  }
  ...
  return {
    ...
    parseMasterM3U8, isMasterM3U8, parseMasterSubtitles, detectVttLanguage,
    formatBitrate, formatFileSize, parseMasterMpd, isMasterMpd
  };
});
